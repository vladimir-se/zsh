...
# Поддержка цыета в выводе команд ls, grep ... .
...
# Переименование-перемещение c пoдтвepждeнueм без коррекции.
alias mv='mv -i'
# Рекурсивное копирование с подтверждением без коррекции.
alias cp='cp -iR'
# Удаление с подтверждением без коррекции.
alias rm='rm -i'
# Принудительное удаление без коррекции.
alias rmf='rm -f'
...
# Автоматический запуск byobu (подключение к ранее созданной сессии)
:<< comments
Подключение к ранее созданной сессии
В новой сессии создается новое окно
При закрытии окна закрывается терминал
Избегается вложенность экземпляров
comments
if [[ $EUID -ne 0 ]]; then
...
# Не выдавать ошибку при несовпадении шаблона (ls *)
setopt +o nomatch

# Приглашение командной строки в mc
